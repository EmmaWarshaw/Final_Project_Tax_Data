...
First we'll run ANOVA to determine if the group proportions are different.  

Next, we'll run two-sample t-tests.  Across the project, we've chosen to use Manhattan as a reference group, so we need to test if the means of every other borough are statistically different to Manhattan.  

```{r}
anova_prop = aov(prop_claims ~ borough, data = nyc_trend)

summary(anova_prop)

TukeyHSD(aov(prop_claims ~ borough, data = nyc_trend))
